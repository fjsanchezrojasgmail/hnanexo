...
(function( window, undefined ) {
    ...
    var Keycloak = function (config) {
        ...
        kc.createLoginUrl = function(options) {
            ...
            if (kc.parsedUri && kc.parsedUri.params) {
                if (kc.parsedUri.params['iss']) {
                    url += '&aud=' + encodeURIComponent(kc.parsedUri.params['iss']);
                }

                // Se vuelven a concatenar a la url, todos los parametros enviados por la url
                for (var prop in kc.parsedUri.params) {
                    // Prevents accidental iteration over properties inherited from an object’s prototype
                    if (kc.parsedUri.params.hasOwnProperty(prop)) {
                        url += '&' + prop + '=' + encodeURIComponent(kc.parsedUri.params[prop]);
                    }
                }
            }
        ...
        kc.updateToken = function(minValidity) {
            ...
            var exec = function() {
                ...
                if (!refreshToken) {
                    ...
                    // Verificamos si el Token ha expirado o si falta minValidity segundos para que expire
                    if(kc.isTokenExpired(0)){
                        // Token expirado: Redirigimos a login
                        console.info('[KEYCLOAK] Token expired. Redirect to login');
                        kc.logout();
                    }else{
                        // Falta minValidity segundos para que el token expire: solicitamos un nuevo token
                        console.info('[KEYCLOAK] Refreshing token');
                ...
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        if (req.status == 200 || fileLoaded(req)) {
                            ...
                            kc.minValidity = config['token-minimum-time-to-live'];
                ...
                return {
                    ...
                    redirectUri: function(options, encodeHash) {
                        ...
                        if (options && options.redirectUri) {
                            ...
                            // [SRP] -> To avoid the 404. Solo nos quedamos con la primera cadena del path
                            var path = location.pathname;
                            // Solo la primera parte del path, no el resto que genera el routing de angular
                            if (location.pathname.indexOf("/", 1) > 0) {
                                path = location.pathname.substring(0, location.pathname.lastIndexOf("/"));
                            }
                            var redirectUri = location.origin + path;
        ...
        var CallbackParser = function(uriToParse, responseMode) {
            ...
            var initialParse = function() {
                ...
                kc.parsedUri = { baseUri: baseUri, queryString: queryString, fragmentString: fragmentString };
                return kc.parsedUri;
            }
            ...
            var parseParams = function(paramString) {
                ...
                for (var i = 0; i < params.length; i++) {
					...
					if (p[0]) {
						var paramName = decodeURIComponent(p[0].replace(/\+/g, ' '));
